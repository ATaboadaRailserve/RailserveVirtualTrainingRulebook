...
// Extended variables - Kyle
private var fogObject : GameObject;
private var timeChanging : boolean;
private var fog : boolean;
private var fogDensitySlider : float;
...
function Awake(){
	fogObject = GameObject.Find("FogParticles");
	if (fogObject) {
		Debug.Log ("Fog found!");
	} else {
		Debug.Log ("Fog not detected");
	}
Helpers();
...
//CAMERA CODE
	...
	//Stop the rain - Kyle
	rainlight = true;
	Weather.RainSettings.randomNumber = 100;
	LightSettings.Sun.SetActive(true);
}

...
SeasonsGame();
	if (timeChanging) {
		if (ActualTimeGame.timeSpeed == 0) {
			divTime = 33000;
			ActualTimeGame.timeSpeed = 2000;
		} else {
			divTime = 52000;
			ActualTimeGame.timeSpeed = 0;
		}
		timeChanging = false;
	}
...
// Extended Functions - Kyle
function GetTime (target : GameObject) {
	target.SendMessage("Time", ActualTimeGame.hour);
}

function SetDensity (fogDense : float) {
	fogDensitySlider = fogDense;
	if (fog) {
		FogSettings.density = fogDensitySlider;
		FogSettings.startDistance = (0.11f - fogDensitySlider)*100f;
		FogSettings.endDistance = (0.11f - fogDensitySlider)*1000f;
		RenderSettings.fogDensity = FogSettings.density;
		RenderSettings.fogStartDistance = FogSettings.startDistance;
		RenderSettings.fogEndDistance = FogSettings.endDistance;
	}
...
function MakeItRain (state : boolean) {
	if (state) {
		rainlight = false;
		Weather.RainSettings.randomNumber = 0;
	} else {
		rainlight = true;
		Weather.RainSettings.randomNumber = 100;
		LightSettings.Sun.SetActive(true);
...
public function SetFog () {
	...
	if (fog) {
		...
		FogSettings.density = 0.0f;
		FogSettings.startDistance = 1000;
		FogSettings.endDistance = 3000;
	}
	RenderSettings.fogDensity = FogSettings.density;
	RenderSettings.fogStartDistance = FogSettings.startDistance;
	RenderSettings.fogEndDistance = FogSettings.endDistance;
